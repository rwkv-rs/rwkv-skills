...
目标：**前端（`client/`）与后端（`src/`）严格分离、命名语义化、Web 层与纯逻辑层分层**，并清除历史残留。
...
- `.gitignore`：移除 `.next/`、`next-env.d.ts`、`.vercel/`；把 `# Node/Next frontend` 注释改为 `# Node/Vite frontend`；新增 `client/dist/`、`*.tsbuildinfo` 已覆盖
...
3. 重写 import：
   ...
   - `web/api.py` 中 `_REPO_ROOT = Path(__file__).resolve().parents[2]` → 因层级加深改为 `parents[3]`，`_SPA_DIST` 指向 `client/dist` 不变（需重算层级）
...
- **验证**：
  ...
  - `pnpm --dir client build` 后访问 `/` 返回 SPA
...
- README（中/英）新增"项目结构"小节，说明 `client/` + `src/dashboard/{web,core}` 布局
