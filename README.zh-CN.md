...
- 数据集准备：`prepare_dataset("livecodebench", Path("data"))` 会下载 LiveCodeBench release_latest（lite）并写出 `data/livecodebench/test.jsonl`。
- 版本数据集：`livecodebench_v1` ... `livecodebench_v6`（lite）。
- 运行 CLI：
  ```bash
