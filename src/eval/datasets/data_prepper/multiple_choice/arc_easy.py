...
def _parse_row(row: dict[str, Any]) -> dict[str, Any]:
    ...
    if not question:
        raise ValueError("ARC-Easy contains an empty question")
    if not isinstance(choices, dict):
        raise ValueError("ARC-Easy choices must be an object")

    texts = choices.get("text")
    labels = choices.get("label")
    if not isinstance(texts, list) or not isinstance(labels, list) or len(texts) != len(labels):
        raise ValueError("ARC-Easy choice texts and labels must be equal-length lists")
    if not 2 <= len(texts) <= 26:
        raise ValueError(f"ARC-Easy contains {len(texts)} choices")
    ...
    try:
        ...
        raise ValueError(f"ARC-Easy answer {answer_key!r} is not in {source_labels!r}") from exc
    ...
    for index, text in enumerate(texts):
        ...
        if not choice:
            raise ValueError(f"ARC-Easy contains an empty choice at index {index}")
...
def _load_records(source_root: Path, split: str) -> list[dict[str, Any]]:
    if split != "test":
        raise ValueError("ARC-Easy benchmark only supports the test split")
...
@MULTIPLE_CHOICE_REGISTRY.register_spec("arc_easy")
def prepare_arc_easy_spec(output_root: Path, split: str = "test") -> HfRepoJsonlDatasetSpec:
