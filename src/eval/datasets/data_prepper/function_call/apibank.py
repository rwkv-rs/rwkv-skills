...
from src.eval.function_calling.agent.adapters.apibank import (
    load_apibank_level2_rows_from_source_dir as load_apibank_level2_agent_rows_from_source_dir,
)
...
def _prepare_apibank_level2_agent(output_root: Path, dataset_name: str) -> list[Path]:
    root = apibank_source_root()
    source_dir = root / "lv1-lv2-samples" / "level-2-toolsearcher"
    rows = load_apibank_level2_agent_rows_from_source_dir(
        source_dir,
        official_root=root,
        dataset_name=dataset_name,
    )
...
def prepare_apibank_level2(output_root: Path, split: str = "test") -> list[Path]:
    ...
    return _prepare_apibank_level2_agent(output_root, "apibank_level2")
