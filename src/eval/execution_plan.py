...
class AttemptKey:
    ...
    def as_tuple(self) -> tuple[int, int, int]:
        return (int(self.sample_index), int(self.repeat_index), int(self.pass_index))
...
def build_attempt_keys(
    plan: AvgKExecutionPlan,
    *,
    max_pass_k: int = 1,
) -> tuple[AttemptKey, ...]:
    if max_pass_k <= 0:
        raise ValueError(f"max_pass_k must be > 0, got {max_pass_k}")
    return tuple(
        AttemptKey(
        ...
        for pass_index in range(max_pass_k)
    )
...
def plan_attempt_count(
    ...
    *,
    ...
    if max_pass_k <= 0:
        raise ValueError(f"max_pass_k must be > 0, got {max_pass_k}")
    return plan.effective_sample_count * int(max_pass_k)


def select_items_by_indices(items: Sequence[T], indices: Iterable[int]) -> list[tuple[int, T]]:
    selected: list[tuple[int, T]] = []
    for index in indices:
        selected.append((int(index), items[int(index)]))
    return selected
