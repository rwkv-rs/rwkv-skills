...
def _load_existing_keys(path: Path) -> set[tuple[str, int, int, int]]:
    """Return {(dataset_split, sample_index, repeat_index, pass_index)} already written."""
    if not path.exists():
        return set()
    ...
    for row in iter_jsonl(path, loads=orjson.loads):
        ...
        pass_index = strict_nonneg_int(row.get("pass_index", 0), "pass_index")
        keys.add((split, sample_index, repeat_index, pass_index))
    return keys
...
def _row_context_text(value: Any) -> str:
    if isinstance(value, str):
        return value
    try:
...
def _row_key(row: dict[str, Any]) -> tuple[str, int, int, int]:
    return (
        ...
        strict_nonneg_int(row.get("pass_index", 0), "pass_index"),
    )


def _build_checker_row(row: dict[str, Any], checked: dict[str, Any]) -> dict[str, Any]:
    return {
...
def _describe_row(row: dict[str, Any]) -> str:
    return (
        ...
        f"pass_index={row.get('pass_index',0)}"
    )
...
def run_llm_checker_rows(
    ...
    *,
    ...
    max_workers = min(max(1, int(cfg.max_workers)), len(rows))
    retry_forever = int(cfg.max_retries) < 0

    def _check_one(row: dict[str, Any]) -> dict[str, Any]:
        context = _truncate_text(_row_context_text(row.get("context", "")), max_chars=int(cfg.max_prompt_chars))
        ...
        return _build_checker_row(row, checked)

    outputs: list[dict[str, Any]] = []
    failed = 0
    stop_scheduling = False
    todo = deque(rows)
    retry_heap: list[tuple[float, tuple[str, int, int, int], dict[str, Any]]] = []
    attempts_by_key: dict[tuple[str, int, int, int], int] = {}
    ...
    if failed and not outputs:
        print(f"⚠️  LLM checker: no new rows produced (failed={failed})")
    return outputs
...
def run_llm_checker(
    ...
    *,
    ...
    output_rows = run_llm_checker_rows(to_check, config=cfg)
    wrote = len(output_rows)
    if wrote:
