...
class _FallbackOnlyBackend:
    ...
    def score_choice_tokens(self, *, prompt: str, choice_token_texts):
        raise AssertionError("multiple-choice evaluation should use generated answers, not choice logits")


def test_multiple_choice_pipeline_uses_generated_answers_by_default(tmp_path) -> None:
    ...
    assert result.payloads[0]["stop_reason1"] == "generated_answer"
    assert len(backend.generate_calls) == 1


def test_multiple_choice_pipeline_batches_answer_generation(tmp_path) -> None:
...
def test_multiple_choice_cot_uses_generated_final_answer_by_default(tmp_path) -> None:
    ...
    assert result.payloads[0]["stop_reason2"] == "generated_answer"
