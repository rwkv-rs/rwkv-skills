...
def test_eval_ingest_dedupes_by_completion(monkeypatch) -> None:
...
def test_eval_strategy_groups_use_separate_tasks(monkeypatch) -> None:
    ...
    task_ids = service.ingest_eval_payload_groups(
        task_id="123",
        completion_payloads=[_payload("completion")],
        payloads_by_group={
...
class _FakeRepo:
    ...
    def fetch_existing_eval_completion_ids(self, _session, *, task_id: int):
        assert task_id == 123
